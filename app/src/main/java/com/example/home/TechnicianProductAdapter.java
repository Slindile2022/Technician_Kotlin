...
public class TechnicianProductAdapter extends RecyclerView.Adapter<TechnicianProductAdapter.ViewHolder> implements Filterable {

    Context context;
    List<ModelClass> modelClassesList, filterList;

    FilterProductsTechnician filter;


    //getting constructor


    public TechnicianProductAdapter(Context context, List<ModelClass> modelClassesList) {
    ...
    public Filter getFilter() {
        if (filter == null){
            filter = new FilterProductsTechnician(this, filterList);
        }
        return filter;
    }
